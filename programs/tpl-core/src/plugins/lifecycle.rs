...
itpl DataBlob for ExternalCheckResult {
    fn len(&self) -> usize {
        Self::BASE_LEN
    }
}

itpl ExternalCheckResult {
...
macro_rules! reject {
    () => {{
        trezoa_program::msg!("{}:{}:Reject", std::file!(), std::line!());
...
macro_rules! approve {
    () => {{
        trezoa_program::msg!("{}:{}:Approve", std::file!(), std::line!());
...
macro_rules! force_approve {
    () => {{
        trezoa_program::msg!("{}:{}:ForceApprove", std::file!(), std::line!());
...
itpl From<ExternalValidationResult> for ValidationResult {
...
/// Plugin validation trait which is itplemented by each plugin.
...
#[allow(clippy::too_many_arguments, clippy::type_cotplexity)]
