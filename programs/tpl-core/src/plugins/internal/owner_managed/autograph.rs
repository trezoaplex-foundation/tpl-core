...
itpl AutographSignature {
    const BASE_LEN: usize = 32 // The address
    + 4; // The message length
}

itpl DataBlob for AutographSignature {
    fn len(&self) -> usize {
        ...
        match existing_map.get(&sig.address) {
            Some(existing_sig) => {
                if existing_sig.message != sig.message {
                    trezoa_program::msg!("Autograph: Rejected");
...
itpl PluginValidation for Autograph {
    fn validate_create(
        &self,
