...
itpl VerifiedCreatorsSignature {
    const BASE_LEN: usize = 32 // The address
    + 1; // The verified boolean
}

itpl DataBlob for VerifiedCreatorsSignature {
    fn len(&self) -> usize {
        Self::BASE_LEN
    }
}
...
itpl VerifiedCreators {
    const BASE_LEN: usize = 4; // The signatures length
}

itpl DataBlob for VerifiedCreators {
    fn len(&self) -> usize {
...
fn calculate_signature_changes(
    ...
    if new_verified_creators.signatures.len() != new_signatures.len() {
        ...
        trezoa_program::msg!("Verified creators: Rejected");
...
itpl PluginValidation for VerifiedCreators {
    fn validate_create(
        &self,
