...
use crate::{
    ...
    state::{AssetV1, DataBlob, TrezoaAccount},
};
...
itpl ExternalPluginAdapterType {
    /// A u8 enum.
    const BASE_LEN: usize = 1;
}

itpl DataBlob for ExternalPluginAdapterType {
    fn len(&self) -> usize {
        Self::BASE_LEN
    }
}
...
itpl From<&ExternalPluginAdapterInitInfo> for ExternalPluginAdapterType {
...
itpl From<&ExternalPluginAdapter> for ExternalPluginAdapterType {
...
itpl ExternalPluginAdapter {
    ...
    pub(crate) fn validate_create(
        ...
        trezoa_program::msg!("ExternalPluginAdapter::validate_create");
        match external_plugin_adapter {
            ExternalPluginAdapter::LifecycleHook(lifecycle_hook) => {
    ...
    pub(crate) fn validate_update_external_plugin_adapter(
        ...
        {
            trezoa_program::msg!("{}:{}:Base:Approved", std::file!(), std::line!());
...
itpl From<&ExternalPluginAdapterInitInfo> for ExternalPluginAdapter {
    fn from(init_info: &ExternalPluginAdapterInitInfo) -> Self {
        match init_info {
            ExternalPluginAdapterInitInfo::LifecycleHook(init_info) => {
...
itpl HookableLifecycleEvent {
    /// A u8 enum.
    const BASE_LEN: usize = 1;
}

itpl DataBlob for HookableLifecycleEvent {
    fn len(&self) -> usize {
        Self::BASE_LEN
    }
}
...
pub enum ExtraAccount {
    ...
    /// Collection-based PDA with seeds \["tpl-core", collection_pubkey\]
    ...
    /// Owner-based PDA with seeds \["tpl-core", owner_pubkey\]
    PreconfiguredOwner {
        /// Account is a signer
        is_signer: bool,
        /// Account is writable.
    ...
    /// Recipient-based PDA with seeds \["tpl-core", recipient_pubkey\]
    ...
    /// Asset-based PDA with seeds \["tpl-core", asset_pubkey\]
    PreconfiguredAsset {
        /// Account is a signer
        is_signer: bool,
        /// Account is writable.
    ...
    pub(crate) fn derive(
        ...
        match self {
            ExtraAccount::PreconfiguredProgram { .. } => {
                let seeds = &[TPL_CORE_PREFIX.as_bytes()];
            ...
            ExtraAccount::PreconfiguredCollection { .. } => {
                let collection = ctx
                    ...
                    .key;
                let seeds = &[TPL_CORE_PREFIX.as_bytes(), collection.as_ref()];
                let (pubkey, _bump) = Pubkey::find_program_address(seeds, program_id);
                Ok(pubkey)
            }
...
itpl From<&ExternalPluginAdapterInitInfo> for ExternalPluginAdapterKey {
    fn from(init_info: &ExternalPluginAdapterInitInfo) -> Self {
        match init_info {
            ExternalPluginAdapterInitInfo::LifecycleHook(init_info) => {
