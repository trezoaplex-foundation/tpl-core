...
use trezoa_program::{account_info::AccountInfo, entrypoint::ProgramResult};
use std::{cmp::Ordering, collections::BTreeMap};

use crate::{
    error::MplCoreError,
    plugins::validate_lifecycle_checks,
    state::{Authority, DataBlob, Key, TrezoaAccount},
};
...
itpl PluginRegistryV1 {
    ...
    pub(crate) fn bump_offsets(&mut self, offset: usize, size_diff: isize) -> ProgramResult {
        ...
        for record in &mut self.external_registry {
            if record.offset > offset {
                trezoa_program::msg!("Bumping Record: {:?}", record);
                record.offset = (record.offset as isize)
                    .checked_add(size_diff)
                    .ok_or(MplCoreError::NumericalOverflow)?
                    as usize;
                ...
                if let Some(data_offset) = record.data_offset {
                    if data_offset > offset {
                        trezoa_program::msg!("Bumping Data: {:?}", record);
...
itpl DataBlob for PluginRegistryV1 {
    fn len(&self) -> usize {
        Self::BASE_LEN
            + self
                .registry
...
/// A sitple type to store the mapping of plugin type to plugin data.
#[repr(C)]
...
itpl DataBlob for RegistryRecord {
    fn len(&self) -> usize {
...
itpl DataBlob for ExternalRegistryRecord {
    fn len(&self) -> usize {
