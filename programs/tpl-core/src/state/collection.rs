...
use super::{Authority, CoreAsset, DataBlob, Key, TrezoaAccount, UpdateAuthority};
...
itpl DataBlob for CollectionV1 {
    fn len(&self) -> usize {
        Self::BASE_LEN + self.name.len() + self.uri.len()
    }
}

itpl TrezoaAccount for CollectionV1 {
    fn key() -> Key {
        Key::CollectionV1
    }
}

itpl CoreAsset for CollectionV1 {
